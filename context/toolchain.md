...
| package manager | pnpm (workspace)                    | 10.27.0                  | root script から `pnpm -r` で束ねる                       |
| toolchain 入口  | Vite+ (`vp`)                        | **未導入**               | 依存に入れていない。入口は root の `package.json` scripts |
...
**まだ導入していない。** `vp` は依存に無く、`vite.config.ts` も存在しない。
現在の入口は root の `package.json` scripts で、`pnpm -r --sequential run <task>` と
`oxlint` / `oxfmt` を直接叩いている。CI も `pnpm run check` を実行する。

以下は [ADR-0003](../adr/0003-monorepo-and-vite-plus.md) で決めた導入後の姿である。
導入するまでは、この節を実装の説明として読まない。
...
- 導入後も CI から各 package の script を直接叩ける状態を保つ。失敗時の切り分けに要る
