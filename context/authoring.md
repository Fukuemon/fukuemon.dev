...
`h2` が手順の境界になる。
**1 つの `h2` が 1 画面**になり、側柱と前後ボタンで切り替わる。
最初の `h2` より前に本文を書くと、それが「はじめに」の面になる。
...
- 押すと 4 ファイル・**gzip 5.28 MiB / brotli 4.45 MiB** を落とす (2026-08-30 実測)
...
- **実行中は「中断」が出る。** Worker を落として止めるので、重いクエリでも画面は固まらない
- **最初の `h2` より前に置いたパネルは、どの手順も完了にしない**
- 側柱の「いまのテーブル」は、1 度実行するまで出ない。
  行を押すと先頭 20 行が別窓で開く
...
- **ビルド時に SVG へ落とす。** ページに mermaid 本体 (約 300 KiB) を配らない
