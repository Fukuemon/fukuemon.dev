...
status: 実装済み
keywords:
  [Design Tokens, primitive, semantic, Zen Old Mincho, EB Garamond, 縦組み, 罫線, 挿絵, 版面]
governs:
...
技術資料としての硬質さを基礎にし、そこへ**版画的な自然の図版と縦組みの見出し**を足す。
...
| 背景      | `--paper` / `--panel` / `--code-bg` / `--tint-*` | 輪郭に 1px 以上の罫を必須とする            |
...
| 図版を含む本文           | `min(100% - 2 * var(--gutter), 52rem)` | 図と表が読める最小           |
...
| 幅     | 変わること                                                                 |
| ------ | -------------------------------------------------------------------------- |
| 1100px | サイドバーをまとめる。記事の目次と柱、ハンズオンの手順一覧を本文の上へ回す |
| 720px  | 縦組みの見出しを横へ倒す。表から分類の列を隠す                             |
| 520px  | 表から種類の列を隠す                                                       |
...
マーカーは完了 `--green` / 現在 `--rust` + `outline` / 未着手 `inset` の罫。
...
**外部の画像ファイルを持たない。**
すべてその場で描く SVG とし、3 階調を `--wood-1` / `--wood-2` / `--wood-3` から参照する。
...
| このサイトの部品 | `ContentRow` / `MetaLine` / 実行パネル / サイドバー | `apps/web/src/components/` と `styles/site.css` |
