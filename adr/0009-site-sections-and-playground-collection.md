# ADR-0009: サイトを about / blog / playground の 3 面に分け、playground を collection にする
...
- mermaid の配信量を 2026-08-30 に実測した。
  ...
  クライアント側の内訳は、入口 11 KiB に対し必ず引く core chunk が 151 KiB + 147 KiB。
...
| 面         | URL           | 役割                                       |
| ---------- | ------------- | ------------------------------------------ |
| about      | `/`           | 誰が書いているか。外部リンク。各面への入口 |
| blog       | `/blog`       | 記事とハンズオン                           |
| playground | `/playground` | 手順に縛られず動かす場所                   |

**about は独立したページを作らず、トップに畳む。**
別ページに切ると、トップが「入口だけのページ」になって 1 面ぶん無駄になる。
...
面を増やしても、意匠の正本は [design-system](../design/features/design-system/DesignDoc_design-system.md) のままである。
「角丸を使わず、影で奥行きを作らない」「紙に刷った版面の見えを保つ」を about と playground にも適用する。

動きは状態の変化を伝えるときだけに使う。
**about だけを例外にする。** トップの表紙を兼ねる面で、名乗りと一覧が刷り上がる動きを持つ。

例外にする理由は、about が読み手に**判断してもらう面ではない**ためである。
一覧や実行パネルの動きは、読む手を止めさせるとそのぶん判断が遅れる。
about に判断は無い。

例外の範囲も、他の面と同じ規則の中に収める。
...
- [design-system](../design/features/design-system/DesignDoc_design-system.md): 意匠の正本
