...
- 画面の構造は次である。
  ...
  | 位置 | 記事                                | ハンズオン                                  |
  | ---- | ----------------------------------- | ------------------------------------------- |
  | 左   | 側柱 306px。目次 + 同じ種別の一覧    | 側柱 306px。手順一覧 + いまのテーブル       |
  | 中   | 本文                                | 本文 (手順は通し。1 枚ずつ出す)             |
  | 右   | なし                                | なし                                        |

  側柱は畳める。畳むと 46px になり、縦組みの見出しと開閉ボタンだけが残る。
...
### 1. Starlight を採り、Override 8 個で画面案へ寄せる
...
### 2. Starlight を採り、意匠を Starlight の既定へ寄せる
