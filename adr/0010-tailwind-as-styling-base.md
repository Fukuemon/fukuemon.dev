# ADR-0010: Tailwind CSS v4 を意匠の土台にし、部品の CSS を utility へ移す
...
- 素の CSS のままでも成立していた。
  10 ページで約 1000 行、部品ごとに命名済みで、knip と oxlint が未使用を落としている。

## 決定

### 1. Tailwind CSS v4 を採る

意匠の土台を Tailwind にし、**部品の CSS を utility へ移して統一する。**
...
- 標準的でない土台を自前で維持し続けることになる。
- 新しく触る人が最初に読むものが増える。

### 2. トークンだけ `@theme` に寄せ、部品の CSS は残す
...
- 境界が「島かどうか」になり、意匠の都合と無関係な線が引かれる。
...
- **移行中に退行が出る。** 全て「基準値と状態が別の層に分かれた」型である。
  1 面ごとに 40 枚 (10 ページ × 明暗 2 配色 × 2 幅) の画素比較で検出する。
